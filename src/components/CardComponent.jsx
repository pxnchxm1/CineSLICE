  ...
  return (
    <Link  href={`${type}/${item?.id}`} underline="none">
      ...
      <Box  className="MovieDetails">
        ...
        <h4  style={{color:'white' ,fontSize:10 ,fontFamily:"monospace",fontWeight:"normal",textAlign:"center", justifyItem:"center"}}> { new Date(item?.release_date ||item?.first_air_date).getFullYear() || "N/A"}</h4>
         <Stack direction={"row"} justifyContent={'center'} gap={1}  marginBottom={1}>
          <img style={{height:20,width:20}}src="../src/assets/star.png" />
          <h4  style={{color:'white',fontSize:10 ,fontFamily:"monospace",fontWeight:"normal"}}>{item?.vote_average} votes</h4>
         </Stack>
